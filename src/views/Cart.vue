...
import {cartsList, cartDel, cartEdit, cartCheckAll} from '../../src/api/apiInterface'
...
export default {
  ...
  methods: {
    ...
    delCart () {
      // 发送post请求
      cartDel({productId: this.delItem.productId}).then(response => {
        let res = response.data
        if (res.status === '200') {
          this.modalConfirm = false
          this.init()
          this.UPDATE_CART_COUNT(-this.delItem.productNum)
        }
      })
    },
    ...
    toggleCheckAll () {
      ...
      // 向后端发送post请求
      cartCheckAll({checkAll: flag}).then(response => {
        let res = response.data
        if (res.status === '200') {
          console.log('update success')
        }
