...
import {login, logout, checkLogin, getCartCount} from '../api/apiInterface'
  ...
  methods: {
    ...
    login () {
      ...
      login({
        ...
        if (res.status === '200') {
          ...
          this.getCartCount()
          this.SET_NICK_NAME(res.result.userName)
        } else {
          this.errorTip = true
        }
      })
    ...
    getCartCount () {
      getCartCount().then(response => {
        let res = response.data
        if (res.status === '200') {
          this.SET_CART_COUNT(res.result)
        }
      })
    }
  }
